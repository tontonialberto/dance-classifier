...
def transform_points(points, confs, box, include_face_poses, scale_to_bounding_box):
    ...
    if scale_to_bounding_box:
        ...
        # points_scaled = np.where(points_scaled < 0, 0, points_scaled)
...
def draw_frame(mode, img, track_ids, boxes, all_points, all_confs, origin):
    ...
    for id, box, points, confs in zip(track_ids, boxes, all_points, all_confs):
        if mode == "poses":
            ...
            cv2.putText(img, str(id), (int(box["x1"] + origin[0] + 10), int(box["y1"] + origin[1] + 10)), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(0, 255, 0))
        elif mode == "boxes":
            draw_skeleton(img, points, confs, x + origin[0], y + origin[1], scale_width, scale_height)
            cv2.putText(img, str(id), (int(x + origin[0] + (scale_width / 2)), int(origin[1] + scale_height)), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(0, 255, 0))
            x += scale_width
    return img

def draw_video_frame(mode, image, frame_index, frame, origin, scale_to_bounding_box, include_face_poses):
    track_ids = []
    boxes = []
    all_points = []
    all_confs = []
    for person in frame:
        if person.get("track_id"):
            ...
            if box_width == 0 or box_height == 0:
                print(f"WARNING: box width or height is 0. Frame {frame_index}, track_id {person.get('track_id')}")
                continue
            ...
            points, confs = transform_points(np.array([xs, ys]).T, confs, box, include_face_poses, scale_to_bounding_box)
...
def draw_frame_of_multiple_videos(image, videos, frame_index, origins):
    for i, video in enumerate(videos):
        ...
        #     video_name = video_name[:20] + "..." + video_name[-20:]
        cv2.putText(image, video_name, (origins[i][0], origins[i][1] + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.25, color=(255, 255, 255))
        ...
        # draw_video_frame(image, frame_index, frame, origins[i], scale_to_bounding_box, include_face_poses)
        if display_poses:
            draw_video_frame("poses", image, frame_index, frame, origins[i], scale_to_bounding_box=False, include_face_poses=include_face_poses)
        elif video["display_boxes"]:
            draw_video_frame("boxes", image, frame_index, frame, origins[i], scale_to_bounding_box=scale_to_bounding_box, include_face_poses=include_face_poses)
...
def display_videos(videos):
    ...
    for i in range(n_frames):
        ...
        cv2.putText(image, "Frame " + str(i), (0, 355), cv2.FONT_HERSHEY_SIMPLEX, 0.25, color=(255, 255, 255))
...
def load_config(config_path):
    ...
    for i, video in enumerate(config["videos"]):
        ...
        assert Path(video["path"]).exists(), f"Video path {video['path']} does not exist - at index {i}"
    return config

videos = [
    {
        "path": "resources/annotations_medium_dataset/bachata_Bailando Bachata- Chayanne ｜ Daniel y Tom Bachata Groove in Budapest [eN5CCONmJTU].mp4.json",
        "frames": [],
        "scale": False,
        "include_face": True,
        "display_poses": False,
    ...
    {
        "path": "resources/annotations_medium_dataset_linerp_movingavg/bachata_Bailando Bachata- Chayanne ｜ Daniel y Tom Bachata Groove in Budapest [eN5CCONmJTU].mp4.json",
        "frames": [],
        "scale": False,
        "include_face": True,
        "display_poses": False,
    ...
    # {
    #     "path": "resources/annotations_small_dataset/bachata_Janis & Zoé  Bleeding Love - Leona Lewis  Bachata Remix  Bachata Sensual  Couple Dance  Baile.mp4.json",
    #     "frames": [],
    #     "scale": True,
    #     "include_face": True,
    #     "display_poses": False,
    ...
    # },
    # {
    #     "path": "resources/annotations_filtered_linear_avg5_minframes60_minvar0.2_minmax/bachata_Daniel & Desirée - ＂Del mar＂ @ Adam Bachata Festival 2021 [m73tKVaGUTY].mp4.json",
    #     "frames": [],
    #     "scale": True,
    #     "include_face": True,
    #     "display_poses": False,
